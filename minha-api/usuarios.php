<?php
    ...
    if($metodo === "GET"){
        ...
        if(isset($_GET["id"])){
            ...
            responder($usuario);//faz a mesma coisa de cima só que usa uma função já pronta pra várias situações
        }
    }else if($metodo === "POST"){
        //CADASTRAR
        $dados = json_decode(
        ...
        */
        ...
        if(isset($dados["nome"]) && empty($dados["nome"])){//ou seja, eu verifico se o nome foi colocado, porém nunca pode estar vazio, caso contrário o banco vai salvar como vazio
            responder([
